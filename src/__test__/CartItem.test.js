...
describe("cart-item tests", () => {
    test("Test for all the elements rendering successfully inside cart page", async () => {
        let cartContainer;
        await act(async () => {
            const { container } = await render(_jsx(MemoryRouter, { children: _jsx(Cart, {}) }));
        ...
        const cartItemMinusBtn = cartContainer.querySelector(".cart__qty-container button:first-child");
        const cartItemAddBtn = cartContainer.querySelector(".cart__qty-container button:last-child");
        const cartItemDeleteBtn = cartContainer.querySelector(".cart__actions .btn--delete");
