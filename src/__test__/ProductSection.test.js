...
describe("Products Tests", () => {
    test("Testing that product section's 1st page is getting rendered successfully", async () => {
        ...
        const productsHeadings = productsContainer.querySelectorAll(".products__item");
        const productsImages = productsContainer.querySelectorAll(".products__item-img");
        const productsTitles = productsContainer.querySelectorAll(".products__item-title");
        const productsCategoryText = productsContainer.querySelectorAll(".rating__text");
        const productsPrices = productsContainer.querySelectorAll(".products__item-price");
    ...
    test("testing products search", async () => {
        await act(async () => {
            await render(_jsx(ProductSection, {}));
        });
        const searchProductInput = screen.getByPlaceholderText(/search for specific product/i);
